    ...
    for i in range(23, 54, 1):
        ...
        if getType != None :
            ...
            if getStatus != 'v' or getStatus == '불가능' :
                ...
                driver.find_element_by_id('boxCountBox').send_keys(str(getQty))    #총 박스 수량 (발주확정시트 C열의 값)
                driver.find_element_by_id('weightBox').send_keys(str(10))    #총 중량
                driver.find_element_by_id('mixPltCount').send_keys(str(1))   #믹스 팔렛트 수량
                driver.find_element_by_id('contentsBox').send_keys('인형')
                driver.find_element_by_id('box_2').send_keys(str(get2boxCnt))
                driver.find_element_by_id('box_3').send_keys(str(get3boxCnt))
