...
def mainLogin():
    ...
    while 1:
        try:
            ...
            targetShipmentInfoUrl = manageSheet.cell(urlData.row, 4).value
            doc = client.open_by_url(targetDateUrl)
            getShipementInfo = client.open_by_url(targetShipmentInfoUrl)
            break
        except:
    ...
    while 1:
        try:
            ...
            # shipmentSheets = getShipementInfo.worksheets()
            getShipemetData = getShipementInfo.worksheet('밀크런-쉽먼트')
            center_lists = list(filter(None, getShipemetData.col_values(2)))
            center_lists.remove('입고센터')
            center_lists.remove('입고센터')
            center_lists.remove('입고센터')
            break
    ...
    for pickCenter in center_lists:
        while 1:
            try:
        ...
        if getStatus != 'v':
            ...
            # action.send_keys('cj').perform()
            # driver.find_element(By.CLASS_NAME,'chosen-search-input').send_keys('cj')
            ...
            for xCnt in range(1, maximumBox + 1, 1):
                while 1:
                    try:
                        getSendNumber = getShipemetData.cell(56 + int(CenterRow), int(xCnt) + 2).value
                        driver.find_element(By.XPATH,
                            '/html[1]/body[1]/div[1]/div[2]/div[2]/div[2]/div[4]/table/tbody/tr[' + str(
                                xCnt) + ']/td[2]/input').clear()
                        driver.find_element(By.XPATH,
                                            '/html[1]/body[1]/div[1]/div[2]/div[2]/div[2]/div[4]/table/tbody/tr[' + str(
                                                xCnt) + ']/td[2]/input').send_keys(getSendNumber)
